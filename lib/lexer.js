...
Tokenizer.prototype = {
    ...
    lexCDATA: function (stopChars) {
        var input = this.source;
        var ch, val = '', pos = this.cursor;
        while (stopChars.indexOf(ch = input[this.cursor]) === -1) {
            if (ch === '&') {
                var piece = input.substring(this.cursor, 10);
                var entity = (/&[^;]+;/.exec(piece) || [])[0];
                if (!entity) {
                    throw this.newSyntaxError('Invalid XML entity: ' + piece);
                }
                val += input.substring(pos, this.cursor);
                this.cursor += entity.length;
                pos = this.cursor;
                if (entity === '&amp;') {
                    val += '&';
                } else if (entity === '&lt;') {
                    ...
                    val += String.fromCharCode(parseInt(entity.substr(3), 16));
                } else if (entity[1] === '#') {
                    val += String.fromCharCode(parseInt(entity.substr(3), 10));
                } else {
                    throw this.newSyntaxError('Invalid XML entity: ' + entity);
                }
            } else {
                ++this.cursor;
            }
        }
        this.token.value = input.substring(pos, this.cursor);
        this.token.type = XML_CDATA;
    },
     ...
     */
    get: function (scanOperand, keywordIsName, xml) {
        var token;
        if (xml && this.lookahead) throw "PANIC: xml w/ lookahead!";
        ...
        if (ich !== null) {
            ...
            this.lexString(ch);
        } else if (this.scanWhitespace && ['\n', '\r', '\t', ' '].indexOf(ch) !== -1) {
            var pos = this.cursor;
            this.skip();
            token.type = WHITESPACE;
            token.value = this.source.substring(pos, this.cursor);
