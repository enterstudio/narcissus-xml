    ...
    switch (tt) {
      ...
      case IF:
        ...
        this.withContext(x2, function() {
            ...
            n.end = this.cursor;
            if (this.match(ELSE, true)) {
                n.elsePart = this.Statement();
                n.end = this.cursor;
            }
        });
        return n;
...
Pp.PrimaryExpression = function PrimaryExpression() {
    ...
    switch (tt) {
      ...
      case LEFT_PAREN:
        ...
        n.end = this.mustMatch(RIGHT_PAREN).end;
...
Pp.XMLName = function XMLName(node) {
    try {
        // tag name
        if (this.peek() === GT) {
            this.xmlName = this.xmlNamespace = undefined;
            return;
        } else {
            this.mustMatch(IDENTIFIER);
        }
        node.xmlName = this.t.token.value;
        while (this.peek() === MINUS) {
            this.mustMatch(MINUS);
            this.mustMatch(IDENTIFIER);
            node.xmlName += '-'+ this.t.token.value;
        }
        ...
        if (this.peek() === COLON) {
            this.mustMatch(COLON);
            this.mustMatch(IDENTIFIER);
            node.xmlNamespace = node.xmlName;
            node.xmlName = this.t.token.value;
            while (this.peek() === MINUS) {
                this.mustMatch(MINUS);
                this.mustMatch(IDENTIFIER);
            ...
            node.xmlNamespace = undefined;
        }

        // eat whitespace
        if (this.peek() === WHITESPACE) {
            this.mustMatch(WHITESPACE);
        }
    } finally {
        this.t.scanWhitespace = false;
    }
};

Pp.XMLLiteral = function XMLLiteral() {
    if (this.t.token.type === LT) {
        var n = this.newNode({ type: XML_LITERAL });
        this.XMLName(n);

        // parse attributes
        var attrs = [];
        while (this.peek() === IDENTIFIER) {
            var attr = this.newNode({type: XML_ATTRIBUTE });
            this.XMLName(attr);
            this.mustMatch(ASSIGN);

            var ch = this.t.source[this.t.cursor];
            if (ch === '"' || ch === "'") {
                // hack the lexer to not lex as a string
                ++this.t.cursor;
                this.t.get(false, false, [ch]);
                attr.xmlValue = this.t.token.value;
                ++this.t.cursor;
                attr.end = this.t.cursor;
            } else if (this.peek() === LEFT_CURLY) {
                // parse as expression
                this.mustMatch(LEFT_CURLY);
                attr.xmlValue = this.Expression();
                attr.xmlValue.end = this.mustMatch(RIGHT_CURLY).end - 1;
            } else {
                this.fail("expecting attr value; found " + definitions.tokens[this.t.get()]);
                return;
            }
            attrs.push(attr);
        }
        n.xmlAttributes = attrs;
        if (this.peek() === DIV) {
            this.mustMatch(DIV);
            n.end = this.mustMatch(GT).end;
            n.xmlChildren = null;
            return n;
        } else {
          this.mustMatch(GT);
        ...
        var children = [];
        while (true) {
            ch = this.t.source[this.t.cursor];
            if (ch === '{' || ch === '<') {
                this.t.get(true);
            } else {
                this.t.get(false, false, ['{', '<']);
            }
            if (this.t.token.type === LT && this.peek() === DIV) {
                break;
            }
            children.push(this.XMLLiteral());
        }
        n.xmlChildren = children;

        // parse end
        this.mustMatch(DIV);
        var name = {};
        this.XMLName(name);
        if (name.xmlName !== n.xmlName || name.xmlNamespace !== n.xmlNamespace) {
            this.fail("mismatched closing tag. expecting " + n.xmlName + ", got " + name.xmlName);
            return;
        }
        n.end = this.mustMatch(GT).end;
        return n;
    } else if (this.t.token.type === LEFT_CURLY) {
        var start = this.t.token.start;
        var n = this.Expression();
        n.start = start;
        n.end = this.mustMatch(RIGHT_CURLY).end - 1;
        return n;
    } else {
        return this.newNode();
    }
